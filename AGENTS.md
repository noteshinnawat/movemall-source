...
4. **🌐 โครงสร้าง Deployment จริงที่ใช้งานอยู่**:
   ...
   - **ห้ามใช้ catch-all fallback ที่ปลอม session ตอน login/register ล้มเหลว**: `handlePasswordLogin` (`src/pages/LoginPage.tsx`) และ `handleFinalRegister` (`src/pages/RegisterPage.tsx`) เคยจับทุก error จาก API (เชื่อมต่อไม่ได้, รหัสผ่านผิด 401, อีเมลซ้ำ 409 ฯลฯ) แล้วสร้างบัญชี/session ปลอมฝั่ง client ให้ผ่านเข้าไปเลย — ทำให้กรอกรหัสผ่านอะไรก็ "login สำเร็จ" ได้ทั้งที่ไม่มีบัญชีจริงในฐานข้อมูล ตอนนี้แก้แล้วให้ error จริงทุกกรณีโชว์ข้อความผ่าน `errorTranslationKey` แทน (ดู `src/utils/passwordLogin.ts` เป็นแบบอย่างสำหรับ flow login) — ห้ามเพิ่ม fallback ลักษณะนี้กลับมาอีก
   - **Cloudflare Turnstile widget ต้องมี anchor ไม่งั้นจะไปชนกับ UI อื่น**: widget "ยืนยันว่าคุณเป็นมนุษย์" (`src/utils/turnstile.ts`) เป็น singleton ที่ลอย `position:fixed` มุมจอเป็นค่าเริ่มต้น ซึ่งเคยชนกับ `FloatingLiveWidget` และ `MobileBottomNav` บนมือถือจนผู้ใช้กดไม่ได้ — หน้าไหนอยากกำหนดตำแหน่งเอง ให้วาง `<div id="cf-turnstile-anchor" />` ไว้ในจุดที่ต้องการ (ดูตัวอย่างใน `LoginPage.tsx` ใต้ปุ่มเข้าสู่ระบบ) widget จะถูกย้ายเข้าไปแทนที่การลอยแบบ fixed โดยอัตโนมัติ
